...
1. **現在の変更状況を確認** → `git status` で未コミットの変更があるか確認
2. **ユーザーの手動編集を確認** → ユーザーが直接ファイルを編集している可能性を常に考慮
3. **変更内容を保存** → 必要に応じて `git stash` で一時保存
4. **ブランチの目的を確認** → メインサイト作業は `main` ブランチ、ペットサイト作業は `feature/pets-site` ブランチ

**重要:**
- ユーザーが「メインサイトの修正」と言った場合は、まず `main` ブランチに切り替えてから作業を開始すること
- ブランチ切り替え時に変更が失われないよう、必ず stash または commit してから切り替えること
- ユーザーがファイルを手動編集した場合は、その変更を尊重し、上書きしないこと
