...
- ファビコン設定
- **外部サイトへのリンク（`target="_blank"`）には必ず`rel="noopener noreferrer"`を付与する**（リバースタブナビング対策。リンク先のページが`window.opener`経由で元タブを操作できてしまう脆弱性を防ぐ）
  - 記事本文（Markdown内の`<a>`タグ）は基本的に付与済みだが、メインサイト・ペットサイト・英語版などHTMLに直接書かれた外部リンクは漏れやすい。2026-07-29の監査で3ページ68箇所が抜けていた実例あり。新しく外部リンクを追加するたびに確認すること
