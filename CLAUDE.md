...
Unsplashから画像を使用する場合は、外部URLを直接参照せず、必ずローカルに保存してからパスを設定すること。

**記事画像の保存先**: `public/assets/images/articles/`
**ファイル命名規則**: 記事のスラッグと同じ名前（例: `third-place-dagayasando-gbc.jpg`）

**手順**:
1. `Invoke-WebRequest` でUnsplash画像をダウンロードして保存先に保存
2. ファイルサイズを確認（200KB超の場合は最適化サービスを案内）
3. 記事のフロントマターに `/assets/images/articles/ファイル名.jpg` のローカルパスを設定
...
- **1記事あたり2000文字程度**を基本とする（Q&A・まとめ含む）
- 極端に短い（1200文字以下）・長い（3000文字超）記事はAEO・SEO上も不利になるため避ける
- 下書き完成後に文字数を確認し、大幅にずれている場合は加筆・削除して調整すること
...
```
1. カテゴリ：以下の10から選んでください（原則1つ、内容が跨る場合は最大2つまでOK）
   コーヒー / 焙煎 / グルテンフリー / 体験・イベント / ペット / 美容健康 / ビジネス / サードプレイス / 店内 / ダガヤサンドウ
...
ユーザーが「画像を提供する」と答えた場合は、`public/assets/images/articles/` フォルダ内の**最新ファイル（更新日時が最も新しいもの）**を記事用画像として使用すること。ユーザーが事前にこのフォルダへ画像を置いておく運用のため、毎回必ずフォルダを確認してから作業を進めること。
...
**内部リンク（SEO・MEO・AEO対策・毎回必須）：**

新記事を作成するたびに、以下の3つをユーザーの指示がなくても必ず自動実施すること。
...
- 新記事と関連性の高い既存記事を選び、その記事の既存リンク段落（まとめ直前）に1文追記する
- 追記は既存文章の末尾に自然につなげる形で挿入し、独立した段落は作らない
- 逆リンクを追加した既存記事のファイルを必ず保存・報告すること

### ③ タグ設定による「関連記事」サイドバーの連動確認

- 新記事のタグが既存記事と1つ以上共通していることを確認する
- 共通タグがない場合は適切なタグを追加するか、ユーザーに確認する
- タグ共通数が多いほどサイドバーの関連記事スコアが高くなる（`relatedPosts`フィルターの仕組み）

---

### アンカーテキストのワード選定ルール（SEO・MEO・AEO）

アンカーテキスト（リンクの表示文字）は検索エンジン・AIへの文脈シグナルになるため、以下のルールで選定すること。

**❌ 避けるべき曖昧なアンカーテキスト：**
- 「こちら」「詳しくはこちら」「関連記事」「詳細」

**✅ 使うべき具体的なアンカーテキスト（例）：**
- 記事タイトルの核心ワードをそのまま使う（「GBCのグルテンフリーフォカッチャ」など）
- 場所キーワードを含める（「北参道のサードプレイス」「ダガヤサンドウのペット可カフェ」など）
- 検索クエリになりうる形（「愛犬と入れるカフェの選び方」「焙煎体験イベント」など）

**MEO（Googleマップ最適化）向けワード：**
- 「北参道」「千駄ヶ谷」「ダガヤサンドウ」などエリア名を必ずアンカーに含める
- 「カフェ」「コーヒー専門店」「ペット可」など業種・特徴ワードと組み合わせる
- 例：`[北参道のペットと入れるカフェ](/articles/pet-cafe-sendagaya/)`

**AEO（AI検索・音声検索最適化）向けワード：**
- 「〜の方法」「〜の理由」「〜とは」など質問の答えに直結する形
- 例：`[サードプレイスとは何かを解説した記事](/articles/third-place-dagayasando-gbc/)`
- 例：`[コーヒーフレッシュを使わないGBCの本気度](/articles/milk-not-creamer/)`（具体的な事実が伝わる）

**SEO（検索エンジン最適化）向けワード：**
- 被リンク元の記事テーマと、リンク先記事のテーマが自然につながるワードを選ぶ
- 同じアンカーテキストを複数記事で使い回さない（多様性を持たせる）
- 例：ビジネス記事→サードプレイス記事へは「第3の居場所になる理由」、ダガヤサンドウ記事からは「ダガヤサンドウがサードプレイスとして機能する理由」と変える
...
【内部リンク実施報告】
① 新記事→既存: [リンク先タイトル] × N本
② 逆リンク追加: [更新した既存記事ファイル名] × N本
③ タグ共通確認: [共通タグ名] で [記事名] と連動
```
...
- **「客」は使わない**。必ず「お客さん」または「お客さま」を使うこと。文脈で使い分ける：
  - カジュアル・親しみやすい文脈 → 「お客さん」（例：「お客さんに喜んでもらえる」）
  - 丁寧・フォーマルな文脈 → 「お客さま」（例：「他のお客さまへの配慮」）
  - 「接客」「客席」などの複合語はそのままでOK
